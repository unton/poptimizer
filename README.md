...
исправлять ошибки, выявленные другими пользователями, и буду рад разумным ПРам.
...
- пакетный менеджер [Scoop](https://scoop.sh/)
- современный аналог `Make` [Task](https://taskfile.dev/installation/)
- фреймворк для JavaScript [NodeJS](https://nodejs.org/en/download)
- сервер базы данных [MongoDb Server](https://www.mongodb.com/try/download/community)
- фреймворк для CUDA [CUDA Toolkit](https://developer.nvidia.com/cuda-12-4-0-download-archive) (достаточно установить runtime)
...
Данная команда предназначена для неопытных пользователей, которые не меняю сами код программы
...
Прогнозы по всем моделям агрегируются и выводятся на вкладке `Forecast` в пересчете в годовое выражение, при этом используется портфельная теория для расчета риска и доходности портфеля на основе характеристик отдельных позиций
